...
export const registerUser = async (values: { email: string; password: string }) => {
    try {
        const response = await axiosInstance.post('http://localhost:2137/auth/register', values);
...
export const login = async (values: { email: string; password: string }) => {
    try {
        const response = await axiosInstance.post('http://localhost:2137/auth/login', values);
...
export const fetchAllUsers = async () => {
    try {
        const response = await axiosInstance.get('/auth/get_all_users'); // Nowa ścieżka
        return response.data;
    } catch (error) {
